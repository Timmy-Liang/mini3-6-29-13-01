...
int heuristic(OthelloBoard BD){
    int edge=0;
    if(BD.board[0][0]==player)edge+=10;
    //else if(BD.board[0][0]==3-player)edge-=5;
    if(BD.board[7][0]==player)edge+=10;
    //else if(BD.board[7][0]==3-player)edge-=5;
    if(BD.board[0][7]==player)edge+=10;
    //else if(BD.board[0][7]==3-player)edge-=5;
    if(BD.board[7][7]==player)edge+=10;
    //else if(BD.board[7][7]==3-player)edge-=5;
    /*
    for(int i=0;i<SIZE;i++){
        if(BD.board[i][0]!=player)break;
        edge++;
        if(i==7)edge+=15;
    }
    for(int i=0;i<SIZE;i++){
        if(BD.board[i][7]!=player)break;
        edge++;
        if(i==7)edge+=15;
    }
    for(int i=0;i<SIZE;i++){
        if(BD.board[0][i]!=player)break;
        edge++;
        if(i==7)edge+=15;
    }
    for(int i=0;i<SIZE;i++){
        if(BD.board[7][i]!=player)break;
        edge++;
        if(i==7)edge+=15;
    }*/
    ...
    for(int i=2;i<=5;i++){
        for(int j=2;j<=5;j++){
            if(BD.board[i][j]==player)me++;
            else if(BD.board[i][j]==3-player)opp++;
        }
    }
    inbox=(me-opp);
    int win=0;
...
//
// 
    return 2*diff+edge+3*valid+inbox;
}
